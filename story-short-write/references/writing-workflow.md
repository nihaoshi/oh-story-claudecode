...
**MiMo Code 适配**：使用 `question` 工具做决策，用 `actor` 做并行任务，用 `memory` 做状态持久化。
...
设计完成后，用 memory 存储关键状态：

```
# 存储角色状态
memory({
  operation: "search",
  query: "character-states {项目名}",
  scope: "projects"
})
...
精修完成后，更新 memory 中的状态：

```
# 更新写作进度
memory({
  operation: "search",
  query: "writing-progress {项目名}",
  scope: "projects"
})
```
